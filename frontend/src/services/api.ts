...
export async function implementProposals(jobId: string, proposalIndices: number[]): Promise<Job> {
...
export async function getDiff(jobId: string, proposalIndex: number): Promise<string> {
  const res = await fetch(`${BASE_URL}/jobs/${jobId}/proposals/${proposalIndex}/diff`)
